...
gprobeKey['aboveFocus'] = [
    10, 4, True, 400, 400, 10, 0, -.1, -.1, -400, 'GUIDE'
]
gprobeKey['belowFocus'] = [
    10, 5, True, 500, 500, 10, 0, -.1, -.1, +400, 'GUIDE'
]
...
class TestGuiderState(guiderTester.GuiderTester, unittest.TestCase):
    ...
    def _setDecenter(self, decenters, enable=None, new=True):
        ...
        self.assertEqual(self.gState.decenterRot,
                         decenters.get('decenterRot', 0))
    ...
    def test_reset_pid_terms_all(self):
        ...
        for axis in ['raDec', 'rot', 'focus', 'scale']:
            self.assertIsNone(self.gState.pid[axis]._x,
                              "%s was not reset" % axis)
    ...
    def test_reset_pid_terms_rot(self):
        ...
        for axis in ['raDec', 'focus', 'scale']:
            self.assertIsNotNone(self.gState.pid[axis]._x,
                                 "%s should not have reset" % axis)

    def _scale_pid_with_alt(self, alt, expect_Ti):
        result = self.gState.scale_pid_with_alt(alt)
        for axis in expect_Ti:
            self.assertEqual(self.gState.pid[axis].Ti, expect_Ti[axis],
                             "%s does not match" % axis)
        return result
