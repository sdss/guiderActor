...
class TestExpose(guiderTester.GuiderThreadTester, unittest.TestCase):
    ...
    def _expose(self, expTime, didFail=False, expect={}, **kwargs):
        gcameraThread.expose(self.cmd, self.actorState, self.replyQueue,
                             expTime, **kwargs)
        msg = self._queue_get(self.replyQueue)
        camera = kwargs.get('camera', 'gcamera')
        self.assertEqual(msg.type,
                         expect.get('expType',
                                    guiderActor.Msg.EXPOSURE_FINISHED))
        self.assertEqual(
            msg.filename,
            self.actorState.models[camera].keyVarDict["filename"][0])
    ...
    def test_flat_5(self):
        self._expose(
            ...
            expType='flat',
            cartridge=10,
            expect={'expType': guiderActor.Msg.FLAT_FINISHED})
    ...
    def test_dark_10_stack_9(self):
        self._expose(
            ...
            stack=9,
            expType='dark',
            expect={'expType': guiderActor.Msg.DARK_FINISHED})
