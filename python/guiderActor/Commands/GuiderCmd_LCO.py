...
def getGprobeKeys():
    ...
    gprobes = yanny.yanny(
        os.path.join(prodDir, 'etc/gcamFiberInfo_LCO.par'), np=True)['GPROBE']
    gprobeFields = [
        "cartridgeId",
        "gProbeId",
        "exists",
    ...
    for gProbeKey in gprobeKeysNumpy:
        gprobeKeys.append([
            castFunc(val)
            for val, castFunc in itertools.izip(gProbeKey, castMap)
        ])
    return gprobeKeys
...
def getGuideInfoKey(gProbeId, guideNumber, plYanny):
    ...
    indexGuide = np.where((objs["holeType"] == "GUIDE") &
                          (objs["fiberId"] == guideNumber))
    indexAlign = np.where((objs["holeType"] == "ALIGNMENT") &
                          (objs["fiberId"] == guideNumber))
    attrList = ["ra", "dec", "xFocal", "yFocal", "phi",
                "throughput"]  # order matters
    for attr in attrList:
        if attr == "phi":
            ...
            guideY = guideInfo[4]
            phi = 90. - math.atan2(
                float(alignY) - float(guideY),
                float(alignX) - float(guideX)) * 180 / math.pi
...
class GuiderCmd_LCO(GuiderCmd.GuiderCmd):
    ...
    def __init__(self, actor):
        ...
        self.keys = keys.KeysDictionary(
            ...
            keys.Key(
                "fiberPos", types.Int(), help="A fiber position, 1-indexed"),
            keys.Key(
                "pmDir",
                types.String(),
                help="Directory where the plPlugMapP files are."))
        # Define new commands for APO
        self.vocab = [
            ("fakeCartridge", "<plate> <fiberPos> <pointing> [<pmDir>]",
             self.fakeCartridge),
        ]
    ...
    def fakeCartridge(self, cmd):
        ...
        fiberPos = int(cmd.cmd.keywords["fiberPos"].values[
            0]) if "fiberPos" in cmd.cmd.keywords else 1
        if fiberPos not in [1, 2, 3]:
            cmd.fail(
                "text=\"fiberPos parameter must be 1, 2 or 3 in fakeCartridge\""
            )
            return
        ...
        if pointing not in self.validpointings:
            cmd.fail(
                "text=\"pointing parameter must be one of %s in fakeCartridge\""
                % ", ".join(self.validpointings))
            return
        plPlugMapDir = cmd.cmd.keywords["pmDir"].values[
            0] if "pmDir" in cmd.cmd.keywords else "/data/plPlugMap/"

        queue = myGlobals.actorState.queues[guiderActor.MASTER]

        # get the path to the plPlugMap file
        pmFilePath = os.path.join(plPlugMapDir, "plPlugMapP-%i%s.par" %
        ...
        guideNums = np.arange(guideStartNum,
                              guideStartNum + 16)  # 16 guide fibers
        gprobes = {}
        for guideNum, gProbeKey in itertools.izip(guideNums, self.gprobekeys):
            ...
            cmd.diag('text="probeNum,guideNum: {},{}"'.format(
                gprobeId, guideNum))
            guideInfoKey = getGuideInfoKey(gprobeId, guideNum, plYanny)
            gProbe = GuiderState.GProbe(
                id=gprobeId, gprobeKey=gProbeKey, guideInfoKey=guideInfoKey)
        ...
        for wavelength in (16600, ):
            try:
                path = os.path.join(
                    ...
                    'plateGuideOffsets-%06d-p%d-l%05d.par' % (plate,
                                                              pointingID,
                                                              wavelength))
            except:
                cmd.warn(
                    'text="no refraction corrections for plate %d at %dA, could not locate PLATELIST_DIR"'
                    % (plate, wavelength))
                continue
            if not os.path.exists(path):
                cmd.warn(
                    'text="no refraction corrections for plate %d at %dA"' %
                    (plate, wavelength))
                continue
            ...
            try:
                ...
                cmd.warn(
                    'text="failed to read plateGuideOffsets file %s: %s"' %
                    (path, e))
                continue
