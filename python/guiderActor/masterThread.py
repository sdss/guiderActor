...
import gimg.guideImage import gi
reload(gi)
...
def getGoodFibers(fibers, guideCmd, gState):
	...
	for fiber in fibers:
		...
		isnan = numpy.isnan
		if isnan(fiber.dx) or isnan(fiber.dy) or isnan(fiber.xyserr):
		...
		try:
			...
			guideCmd.warn('text="Gprobe %d was not listed in plPlugMapM file"' % fiber.fiberid)
			continue

		theta = math.radians(theta)
		ct, st = math.cos(theta), math.sin(theta)
		dRA   =  fiber.dx*ct + fiber.dy*st # error in guide star position; n.b. still in mm here
		dDec  = -fiber.dx*st + fiber.dy*ct
		dDec *= -1
		fiber.dRA = dRA
		fiber.dDec = dDec
		#print 'dRA,dDec (in mm):', (dRA, dDec)

		guideCmd.inform("star=%d,%2d, %7.2f,%7.2f, %7.2f,%7.2f, %6.1f,%6.1f,  %6.1f,%6.1f, %6.1f,%6.1f,%4.1f, %7.3f,%4.0f" % (
			frameNo, fiber.fiberid,
			dRA, dDec, 
			fiber.dx, fiber.dy,
			fiber.xs, fiber.ys, 
			probe.xCenter, probe.yCenter,
			probe.xFocal, probe.yFocal, probe.rotStar2Sky,
			fiber.fwhm/sigmaToFWHM, probe.focusOffset))

		print "%d %2d  %7.2f %7.2f  %7.2f %7.2f  %6.1f %6.1f  %6.1f %6.1f  %6.1f %6.1f  %06.1f  %7.3f %4.0f" % (
			frameNo,
			fiber.fiberid, dRA, dDec, fiber.dx, fiber.dy, fiber.xs, fiber.ys, probe.xCenter, probe.yCenter,
			probe.xFocal, probe.yFocal, probe.rotStar2Sky,
			fiber.fwhm/sigmaToFWHM, probe.focusOffset)
		...
		goodfibers.append(fiber)
	return goodfibers

def solveForRaDecRotScale(fibers, guideCmd):
	A = numpy.matrix(numpy.zeros(3*3).reshape([3,3]))
	b = numpy.matrix(numpy.zeros(3).reshape([3,1]))
	b3 = 0.0

	guideRMS    = 0.0
	for fiber in fibers:
		raCenter  = fiber.probe.xFocal
		decCenter = fiber.probe.yFocal
		dRA  = fiber.dRA
		dDec = fiber.dDec
	...
	if nStar == 1:
		...
		guideRMS = math.sqrt(guideRMS / nStar)

	return (x[0,0], x[1,0], x[2,0], dScale, guideRMS, nStar)

def solveForFocus(fibers, gState, guideCmd):
    #
	...
	return (dFocus, rms0, success)


def measureGuiderImage(dataname, cartridgeId, gprobes, flatname, darkname,
					   gState, guideCmd,
					   plot, psPlot, oneExposure, force):
	try:
		GI = gi.GuiderImageAnalysis(dataname, guideCmd)
	...
	goodfibers = getGoodFibers(fibers, guideCmd, gState)
	fibers = goodfibers
	...
	if plot or sm:                           #setup arrays for sm
		...
		for f in fibers:
			try:
				fiberid_np  [f.fiberid] = f.fiberid
				raCenter_np [f.fiberid] = f.probe.xFocal
				decCenter_np[f.fiberid] = f.probe.yFocal
				dRA_np      [f.fiberid] = f.dRA
				dDec_np     [f.fiberid] = f.dDec
			except IndexError, e:
	...
	(dRA_mm, dDec_mm, dRot_rad, dScale, guideRMS, nStar) = solveForRaDecRotScale(fibers, guideCmd)
	# Returns zeroes on failure.
	...
	dRA  = dRA_mm  / gState.plugPlateScale
	dDec = dDec_mm / gState.plugPlateScale
	# and from radians to degrees
	dRot = -math.degrees(dRot_rad)
	...
	# FIXME -- move back to main control loop?
	...
	(dFocus,rms0,gotfocus) = solveForFocus(fibers, gState, guideCmd)
    ...
    while True:
        try:
            ...
            if msg.type == Msg.EXIT:
				...
				frameInfo = measureGuiderImage(msg.filename, gState.cartridge, gState.gprobes, flatfile, darkfile,
											   gState, guideCmd, plot, psPlot):
